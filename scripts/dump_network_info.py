...
def get_network_with_key(network_key):
    ...
    if name in ['mobilenet_v1','nasnet','nasnetmobile','nasnetlarge','xception']:
        import torch
        from scripts.e-models import mobilenet_v1,nasnetmobile,xception
        input_shape = args[0]
        dtype = 'float32'
        if name == 'mobilenet_v1':
            model = mobilenet_v1.MobileNetV1()
        if name == 'nasnetmobile':
            model = nasnetmobile.NASNetAMobile()
        if name == 'nasnetlarge':
            model = nasnetlarge.NASNetALarge()
        if name == 'xception':
    ...
    if name in ['resnet_18', 'resnet_50', 'mobilenet_v2', 'mobilenet_v3',
                ...
                'densenet_121', 'vgg_16', 'shufflenetv2']:
        import torch
        ...
        if name in ['resnet_18', 'resnet_50']:
            ...
            model = getattr(models, 'shufflenet_v2_x1_0')(pretrained=False)
...
def build_network_keys():
    ...
    for batch_size in [1, 4, 8]:
        for image_size in [299]:
            network_keys.append((f'xception',
                                [(batch_size, 3, image_size, image_size)]))
    # nasnetmobile
    for batch_size in [1, 2, 4]:
        for image_size in [224, 240, 256]:
            network_keys.append((f'nasnetmobile',
                                [(batch_size, 3, image_size, image_size)]))
    # nasnetmobile
    for batch_size in [1, 2, 4]:
        for image_size in [299,331]:
            network_keys.append((f'nasnetmobile',
                                [(batch_size, 3, image_size, image_size)]))
    # shufflenet_v2
    for batch_size in [1, 4, 8]:
    ...
    for batch_size in [1, 4, 8]:
        for image_size in [224, 240, 256]:
            network_keys.append((f'mobilenet_v1',
                                [(batch_size, 3, image_size, image_size)]))
    # mobilenet_v2_v3
    for batch_size in [1, 4, 8]:
        for image_size in [224, 240, 256]:
